document.addEventListener('DOMContentLoaded', () => {
  ...
  function updateUI() {
    ...
    chrome.storage.local.get(['segments', '_liveState'], (result) => {
      ...
      studyTimeEl.textContent    = formatDuration(savedStudy + (activeType === 'study'       ? liveSecs : 0));
      distractTimeEl.textContent = formatDuration(savedDist  + (activeType === 'distraction' ? liveSecs : 0));
  ...
  doneBtn.addEventListener('click', () => {
    ...
    chrome.runtime.sendMessage({ action: 'endStudyDay' }, () => {
      if (chrome.runtime.lastError) {
        // Background may be asleep — still poll for report since it may wake up
  ...
  function renderReport(r) {
    ...
    reportRows.innerHTML = `
      ...
      <div class="report-row">
        ...
        <span class="report-row-val" style="color:#2b7cd4">${formatDuration(r.totalStudy)}</span>
      </div>
      <div class="report-row">
        <span class="report-row-label">Distraction time</span>
        <span class="report-row-val" style="color:#c0392b">${formatDuration(r.totalDistraction)}</span>
      </div>
    `;
